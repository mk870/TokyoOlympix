...
import {
  ...
  GET_VIDEODATA,
  POST_COMMENT,
} from "../Components/Utilis/GraphqlQuery";
...
import {
  ...
  getFirstTwoLetters,
  getUserName,
} from "../Components/Utilis/UserNameManipulation";
...
const Video = () => {
  ...
  const [deleteData, deleteMutation] = useMutation(DELETE_COMMENT, {
    ...
    onError: (error) => console.log(error),
    onCompleted: () => setPopup("Delete successful"),
  });
  const [editData, editMutation] = useMutation(EDIT_COMMENT, {
    ...
    onError: (error) => console.log(error),
    onCompleted: () => setPopup("Edit successful"),
  });
  ...
  return (
    <div className={styles.wrapper}>
      {clickedVideo ? (
          ...
          <div className={styles.video}>
            ...
            <div className={styles.segment}>
              <h4 className={styles.header}>
                {cleanSnippet(clickedVideo.snippet.title)}
              </h4>
            </div>
            {video.loading && (
            ...
            {video.error && (
              <h4 className={styles.error}>
                Sorry an error occurred fetching comments
              </h4>
            )}
            {video.data && (
              <div className={styles.graphql}>
                ...
                <div className={styles.comments}>
                  <div className={styles.commentsNum}>
                    {`${video.data.commentsByVideoId.length} ${
                      video.data.commentsByVideoId.length === 1
                        ? "Comment"
                        : "Comments"
                    }`}
                  </div>
                  <div className={styles.input}>
                    <div className={styles.name}>
                      {value ? (
                        getFirstTwoLetters(getEmail(value))
                      ) : (
                    ...
                    <div className={styles.addcomment}>
                      ...
                      {comment ? (
                        <div className={styles.btn}>
                          <button
                            className={buttonStyles.commentBtn}
                            onClick={postComment}
                          >
                            <FaComment
                              color="aliceblue"
                  ...
                  deleteMutation.loading ? (
                    ...
                    mutation.error ? (
                    <h4 className={styles.error}>An error occured</h4>
                  ) : (
                    <div className={styles.list}>
                      {video.data.commentsByVideoId.length ? (
                        reverseList(video.data.commentsByVideoId).map(
                          (comment, index) => (
                            <div className={styles.itemComment} key={index}>
                              <div className={styles.comment}>
                                ...
                                <div className={styles.wrap}>
                                  ...
                                  {getEmail(value) === comment.user.email ? (
                                    <div className={styles.edits}>
                                      <button
                                        className={buttonStyles.editbtn}
                                        onClick={() => getComment(comment)}
                                      >
                                        <FaEdit
                                          color="aliceblue"
                                          className={styles.icon}
        ...
        <h1 className={styles.segment2}>No video</h1>
      )}
    </div>
  );
};
